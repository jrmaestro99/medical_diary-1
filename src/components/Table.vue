...
export default {
  ...
  mounted() {
    ...
    this.display(String(this.user))
    this.showReminder()
    // async function mounteddisplay(user) {
    ...
    //       var table = document.getElementById("table").then;
  ...
  methods: {
    ...
    async display(user) {
      ...
      let z = await getDocs(collection(db, user))
        ...
        z.forEach((docs) => {
          ...
          deleteButton.onclick = function() {
           this.confirmDelete(String(medName), user);
          };
        ...
        //dealing with the undefined entries
        tb.deleteRow(1)
        tb.deleteRow(1)
    ...
    async delReminder(reminder, user) {
      await deleteDoc(doc(db, user, reminder))
      console.log("deleted ", reminder);
      // let table = document.getElementById("table");
    ...
    async confirmDelete(reminder, user) {
      let isConfirmed = confirm("Delete this reminder?")
      if (isConfirmed) {
        this.delReminder(reminder, user)
      }
    },
  } 
}
